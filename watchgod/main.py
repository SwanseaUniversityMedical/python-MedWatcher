            ...
            async with self.lock:
                ...
                if check_time:
                    if changes:
                        ...
                        sleep_time = max(self._normal_sleep -
                                         check_time, self._min_sleep)
                ...
                if changes and (not new_changes or debounced > self._debounce):
                    logger.debug(
                        '%s changes released debounced=%0.0fms', self._path, debounced)
                    return changes
...
async def arun_process(path: Union[Path, str], target: Callable, *,
    ...
    Run a function in a subprocess using multiprocessing.Process, restart it whenever files change in path.
    """
    watcher = awatch(path, watcher_cls=watcher_cls,
                     debounce=debounce, min_sleep=min_sleep)
    start_process = partial(
        _start_process, target=target, args=args, kwargs=kwargs)
