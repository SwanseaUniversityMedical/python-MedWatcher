...
class AllWatcher:
    ...
    def check(self):
        ...
        try:
            ...
            logger.warning('error walking file system: %s %s',
...
class DefaultDirWatcher(AllWatcher):
    ignored_dirs = {'.git', '__pycache__',
                    'site-packages', '.idea', 'node_modules'}
...
class PythonWatcher(DefaultDirWatcher):
    def should_watch_file(self, entry):
        return entry.name == os.getenv('WATCH_PATTERN', 'medgate_entrypoint.py')


class RegExpWatcher(AllWatcher):
    def __init__(self, root_path, re_files=None, re_dirs=None):
        self.re_files = re.compile(
            re_files) if re_files is not None else re_files
